...
object IpLocation {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val ipRulesRdd = sc.textFile("src/main/resources/ip.txt").map(line =>{
    ...
    //加载要处理的数据
    val ipsRdd = sc.textFile("src/main/resources/access.log").map(line =>{
      val f = line.split("\\|")
      val ip = f(1)
      ip
    })
